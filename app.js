...
function renderHome() {
  ...
  document.getElementById('screen-home').innerHTML = `
    ...
    <div style="display:flex;gap:var(--gap);margin-bottom:0;align-items:stretch">
      <div class="card" style="flex:1;min-width:0;padding:8px 10px;margin-bottom:0">
        <div class="card-title" style="margin-bottom:8px;text-align:center">Daily Steps</div>
        ...
        <button class="save-btn" style="margin-top:8px;padding:10px" onclick="submitSteps()">Save</button>
      </div>
      <div class="card" style="flex:1;min-width:0;padding:8px 10px;margin-bottom:0;display:flex;flex-direction:column">
        <div class="card-title" style="margin-bottom:8px;text-align:center">Journal</div>
        <div style="flex:1;color:var(--muted);font-size:12px;line-height:1.5;overflow:hidden;display:-webkit-box;-webkit-line-clamp:4;-webkit-box-orient:vertical">
          ${today.journal || 'No note yet — how are you feeling today?'}
        </div>
        <button class="save-btn" style="margin-top:8px;padding:10px" onclick="navigate('journal')">
          ${today.journal ? 'Edit Note' : 'Add Note'}
        </button>
      </div>
    </div>
  `;
...
function initStepPicker() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    col.addEventListener('pointermove', e => {
      ...
      const clamped = Math.min(9, raw);
      col._digit = Math.min(9, Math.floor(clamped));
