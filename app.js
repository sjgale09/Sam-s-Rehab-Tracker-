...
function calcPainStreak() {
  ...
  // Collect consecutive scores walking backwards until a day with no data
  const scores = [];
  ...
  // scores[0] = most recent day, scores[1] = day before, etc.
  // Count consecutive days where each is ≤ the day before it (scores[i] ≤ scores[i+1])
  let streak = 0;
  for (let i = 0; i < scores.length; i++) {
    if (i === scores.length - 1) { streak++; break; }   // oldest day — always counts if we got here
    if (scores[i] <= scores[i + 1]) { streak++; }       // same or better than day before ✓
    else { break; }                                       // worse than day before — streak ends
  }
  return streak;
}
