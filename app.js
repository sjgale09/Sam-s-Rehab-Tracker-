...
const EXERCISE_LIBRARY = [
  ...
  { id: 'big-toe-towel-strengthen', name: 'Big Toe Towel Strengthening', category: 'Toes & Foot', defaultSets: '3 × 10–15', desc: 'Place big toe on a folded towel, bend the knee and lean forward pressing the big toe firmly down into the towel. Strengthens the flexor hallucis and supports arch control.' },
  ...
  // ── Physio Prescribed — Primary ───────────────────────────────────────────
  { id: 'medial-wall-push-iso', name: 'Medial Wall Push ISO', category: 'PTT — Physio Prescribed', defaultSets: '3 × 45–60s hold', desc: 'Stand side-on to a wall, affected foot closest. Rise onto the forefoot and lean body toward the wall so weight transfers through the outer border of the foot while the PTT holds the inner arch isometrically. Toes long and relaxed throughout.' },
  { id: 'wall-facing-push-iso', name: 'Wall Facing Push ISO', category: 'PTT — Physio Prescribed', defaultSets: '3 × 45–60s hold', desc: 'Stand facing a wall. Place the forefoot of the affected foot against the base and push forward into dorsiflexion against the wall\'s resistance without the foot sliding. Hold completely still.' },
  { id: 'split-stance-calf-raise-iso', name: 'Split Stance Calf Raise ISO', category: 'PTT — Physio Prescribed', defaultSets: '3 × 45–60s hold', desc: 'Stand in a split stance with affected foot behind. Rise onto the toes of the back foot and hold at the top without lowering or pulsing.' },
  { id: 'slcr-hold', name: 'Single Leg Calf Raise Hold', category: 'PTT — Physio Prescribed', defaultSets: '3 × 45–60s hold', desc: 'Stand on the affected foot only. Rise onto the toes and hold at the highest comfortable point without bobbing or pulsing, using light wall support for balance.' },
  // ── PTT Isometric — Supplementary ─────────────────────────────────────────
  { id: 'iso-inversion-band', name: 'Isometric Inversion Hold (Band)', category: 'PTT — Isometric', defaultSets: '3 × 45–60s hold', desc: 'Seated, band anchored on the outer side looped around the forefoot. Press the foot inward against the band without moving it.' },
  { id: 'iso-inversion-floor', name: 'Isometric Inversion Floor Press', category: 'PTT — Isometric', defaultSets: '3 × 45–60s hold', desc: 'Seated, press the inner border of the foot firmly downward and inward into the floor as if turning the sole inward, without the foot actually moving.' },
  { id: 'short-foot-iso-seated', name: 'Short Foot ISO (Seated)', category: 'PTT — Isometric', defaultSets: '3 × 45–60s hold', desc: 'Seated, draw the ball of the foot toward the heel without curling the toes to dome the arch. Hold the position statically with toes long and relaxed.' },
  { id: 'short-foot-iso-standing', name: 'Short Foot ISO (Standing)', category: 'PTT — Isometric', defaultSets: '3 × 30–45s hold', desc: 'Standing on the affected foot with light wall support, draw the ball of the foot toward the heel to dome the arch under full bodyweight without curling the toes.' },
  // ── Achilles Isometric ─────────────────────────────────────────────────────
  { id: 'iso-plantarflexion-floor', name: 'Isometric Plantarflexion Hold (Floor)', category: 'Achilles — Isometric', defaultSets: '3 × 45–60s hold', desc: 'Seated, press the forefoot firmly downward into the floor as if pointing the foot without the foot moving.' },
  { id: 'iso-plantarflexion-band', name: 'Isometric Plantarflexion Hold (Band)', category: 'Achilles — Isometric', defaultSets: '3 × 45–60s hold', desc: 'Seated, band anchored behind you looped around the forefoot. Press the foot away into plantarflexion against the band without moving.' },
  // ── Tibialis Anterior Isometric ───────────────────────────────────────────
  { id: 'iso-dorsiflexion-band', name: 'Isometric Dorsiflexion Hold (Band)', category: 'ATT — Isometric', defaultSets: '3 × 45–60s hold', desc: 'Seated, band anchored ahead at floor level looped around the forefoot. Pull the foot upward toward the shin against the band and hold completely still. Do not exceed 60 seconds per hold.' },
  { id: 'iso-dorsiflexion-floor', name: 'Isometric Dorsiflexion Hold (Floor)', category: 'ATT — Isometric', defaultSets: '3 × 45–60s hold', desc: 'Seated, place the unaffected foot lightly on top of the affected forefoot. Attempt to lift the forefoot upward against its resistance without moving.' },
  { id: 'tib-ant-wall-hold', name: 'Tibialis Anterior Wall Hold', category: 'ATT — Isometric', defaultSets: '3 × 30–45s hold', desc: 'Standing facing a wall, place the dorsum of the affected foot against the base. Push upward into dorsiflexion against the wall without the foot moving.' },
  // ── Posterior Chain & Advanced ─────────────────────────────────────────────
  { id: 'iso-eversion-band', name: 'Isometric Eversion Hold (Band)', category: 'Posterior Chain & Advanced', defaultSets: '3 × 45–60s hold', desc: 'Seated, band anchored on the inner side looped around the forefoot. Press the foot outward against the band without moving it to strengthen the peroneals.' },
  { id: 'slwall-sit-iso', name: 'Single Leg Wall Sit ISO', category: 'Posterior Chain & Advanced', defaultSets: '3 × hold to tolerance', desc: 'Back against a wall, slide to approximately 120–130 degrees of knee bend. Transfer weight onto the affected leg and hold. Advanced — introduce only when all prescribed exercises are well established.' },
  { id: 'iso-hip-abduction', name: 'Isometric Hip Abduction Hold', category: 'Posterior Chain & Advanced', defaultSets: '3 × 45–60s hold', desc: 'Lying on your side with affected leg on top, lift the leg to approximately 30 degrees and hold without pulsing. Strengthens hip abductors weakened by crutch use.' },
...
function calcPainStreak() {
  ...
  const todayScore = getEffectivePainScore(state.data[todayKey()]);
  const startOffset = todayScore !== null ? 0 : 1;
  // Collect consecutive logged scores walking backwards (stop at first missing day)
  const scores = [];
  for (let i = startOffset; i <= 365; i++) {
    const d = new Date(today); d.setDate(today.getDate() - i);
    const score = getEffectivePainScore(state.data[d.toISOString().slice(0,10)]);
    if (score === null) break;
    scores.push(score);
  }
  if (scores.length === 0) return 0;
  // scores[0] = most recent, scores[1] = day before, etc.
  // Most recent logged day always anchors a streak of at least 1.
  // Each older day extends the streak only if score[i] >= score[i-1]
  // (i.e. the older day was same or worse, meaning going forward pain stayed same or improved).
  let streak = 1;
...
function submitPainScores() {
  ...
  if (btn) { btn.textContent = '✓ Saved'; btn.classList.add('saved'); setTimeout(() => { btn.textContent = 'Submit Pain Scores'; btn.classList.remove('saved'); }, 1800); }
}
...
function renderHistory() {
  ...
  function buildCalendar(year, month) {
    ...
    for (let day = 1; day <= last.getDate(); day++) {
      ...
      // Check if this day broke the streak (score higher than the most recent previously logged day)
      let brokeStreak = false;
      if (ps !== null) {
        // Walk back up to 60 days using direct date arithmetic (avoids timezone issues with toISOString)
        let prevPs = null;
        for (let offset = 1; offset <= 60; offset++) {
          const prev = new Date(year, month, day - offset); // local date arithmetic
          const py = prev.getFullYear();
          const pm = String(prev.getMonth() + 1).padStart(2, '0');
          const pd = String(prev.getDate()).padStart(2, '0');
          const prevKey = `${py}-${pm}-${pd}`;
          const candidate = getEffectivePainScore(state.data[prevKey]);
          if (candidate !== null) { prevPs = candidate; break; }
        }
        if (prevPs !== null && ps > prevPs) brokeStreak = true;
      }
      const dot = ps !== null
        ? `<svg width="20" height="20" viewBox="0 0 20 20" style="display:block;margin:1px auto 0;overflow:visible">
            <circle cx="10" cy="10" r="9" fill="${painColor(ps)}"/>
            <text x="10" y="14" text-anchor="middle" font-size="8" font-weight="600" fill="white" font-family="inherit">${ps}</text>
            ${brokeStreak ? `<line x1="3" y1="3" x2="17" y2="17" stroke="white" stroke-width="2.5" stroke-linecap="round"/>` : ''}
          </svg>`
    ...
    return `
      ...
      <div style="display:flex;justify-content:flex-end;margin-top:8px">
        <button id="cal-refresh-btn" onclick="refreshCalendar(this)" title="Recalculate streaks" style="background:none;border:1px solid var(--muted);border-radius:20px;padding:4px 12px;font-size:12px;color:var(--muted);cursor:pointer;outline:none;display:inline-flex;align-items:center;gap:5px">
          <span id="cal-refresh-icon" style="font-size:14px;display:inline-block;transition:transform 0.6s ease">↻</span> Refresh streaks
        </button>
      </div>`;
  }
  ...
  function calcLongestPainStreak() {
    ...
    for (const k of allKeys) {
      const score = getEffectivePainScore(state.data[k]);
  ...
  function calcBestStepWeek() {
    ...
    Object.keys(state.data).forEach(k => {
      ...
      if (steps > 0) {
        if (!weekData[wk]) weekData[wk] = { total: 0, count: 0 };
        weekData[wk].total += steps;
        weekData[wk].count++;
      }
    });
    let best = 0, bestWeek = null;
    Object.entries(weekData).forEach(([wk, { total, count }]) => {
      const avg = Math.round(total / count);
      if (avg > best) { best = avg; bestWeek = wk; }
    });
  ...
  const achievementsHtml = `
    ...
    <div class="card">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--gap)">
        ...
        <div style="background:var(--blue-bg);border-radius:var(--radius-sm);padding:10px 12px">
          <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;color:var(--muted);margin-bottom:4px">👟 Best Avg Step Week</div>
...
function refreshCalendar(btn) {
  // Spin the icon, then re-render after animation completes
  const icon = btn ? btn.querySelector('span') : document.getElementById('cal-refresh-icon');
  if (icon) {
    icon.style.transition = 'transform 0.6s ease';
    icon.style.transform = 'rotate(360deg)';
    setTimeout(() => renderHistory(), 650);
  } else {
    renderHistory();
  }
}
